...
function volume(heights) {
  ...
  for (var i = 0; i < containers.length; i++) {
  	var ndxStart = heights.indexOf(containers[i][0]);
  	var ndxEnd = heights.indexOf(containers[i][1]);
  	for (var j = ndxStart+1; j < ndxEnd; j++) {
  		var min = Math.min(heights[ndxStart], heights[ndxEnd]);
  		area += min - heights[j]
  	}
  }
